...
- **Static Export for Cloudflare Pages**: Replaced `@cloudflare/next-on-pages` adapter with Next.js static export (`output: 'export'`). The adapter required all routes to use Edge Runtime, incompatible with Node.js API routes (MongoDB/Mongoose). Since API routes run on Render, Cloudflare only serves the static frontend. Updated `wrangler.toml` output dir from `.vercel/output/static` to `out/`, simplified `cf-build` script, conditionally disabled headers/redirects/rewrites (unsupported with static export), and added `public/_redirects` for SPA fallback routing.
